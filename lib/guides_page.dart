...
const _redFlagKeywords = [
  '호흡곤란', '흉통', '의식 저하', '황달', '심한 출혈', '응급',
  '즉시 병원', '119', '쇼크', '혈압 저하', '심정지',
];
...
class _GuidesPageState extends State<GuidesPage> {
  ...
  @override
  void initState() { super.initState(); _loadGuides(); }

  @override
  void dispose() { _client.close(); super.dispose(); }

  Future<String?> _getToken() async => SecureTokenStorage().getAccessToken();
  ...
  Color _getStatusColor(String? status) {
    switch (status) {
      ...
      case 'needs_update': return Colors.orange;
  ...
  String _getStatusText(String? status) {
    switch (status) {
      ...
      case 'needs_update': return '업데이트 필요';
  ...
  Widget _buildEmpty() {
    return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      const SizedBox(height: 16),
      const Text('안내문이 없습니다.', style: TextStyle(color: Colors.grey, fontSize: 16)),
      const SizedBox(height: 8),
      const Text('진료기록에서 안내문을 생성해보세요.', style: TextStyle(color: Colors.grey, fontSize: 13)),
    ]));
  }
...
class _GuideDetailPageState extends State<GuideDetailPage> {
  ...
  @override
  void initState() { super.initState(); _loadGuide(); }
  ...
  // ✅ NFR-SAFE-001: Red Flag 키워드 감지
  void _checkRedFlags() {
    if (_guide == null) return;
    final allContent = [
      _guide?['medication_guide'], _guide?['lifestyle_guide'],
      _guide?['precautions'], _guide?['recommended_checkups'],
    ].whereType<String>().join(' ');
    final found = _redFlagKeywords.where((kw) => allContent.contains(kw)).toList();
    if (found.isNotEmpty) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) _showRedFlagModal();
      });
    }
  }
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('안내문에 의료진 확인이 필요한 내용이 포함되어 있습니다.',
              style: TextStyle(fontSize: 14, height: 1.5)),
          SizedBox(height: 12),
          Text('즉시 담당 의료진 상담 또는 응급실 방문을 권고합니다.',
  ...
  Future<void> _regenerateGuide() async {
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('안내문이 재생성됐습니다.')));
        _loadGuide();
      } else {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('재생성에 실패했습니다.')));
      }
    } catch (_) {
      if (!mounted) return;
        ...
        builder: (context, setModalState) => Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              const Text('안내문 평가', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              IconButton(icon: const Icon(Icons.close), onPressed: () => Navigator.pop(context)),
            ]),
            const SizedBox(height: 16),
            const Text('이 안내문이 얼마나 도움이 됐나요?', style: TextStyle(fontSize: 14, color: Colors.black87)),
  ...
  Future<void> _submitFeedback(int rating, String comment) async {
    try {
      ...
      final response = await _client.post(
        Uri.parse('${OcrConfig.baseUrl}/v1/guides/${widget.guideId}/feedback'),
        headers: {'Content-Type': 'application/json', 'Authorization': 'Bearer $token'},
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(response.statusCode == 200 || response.statusCode == 201
              ? '평가가 제출됐습니다. 감사합니다!' : '평가 제출에 실패했습니다.')));
    } catch (_) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('오류가 발생했습니다.')));
    }
  ...
  Widget _buildInfoCard() {
    ...
    if (status == 'needs_update') { statusColor = Colors.orange; statusText = '업데이트 필요'; }
    else if (status == 'archived') { statusColor = Colors.grey; statusText = '보관됨'; }
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 8),
        Text('v$version · $formattedDate', style: const TextStyle(color: Colors.grey, fontSize: 13)),
      ]),
    );
  }
  ...
  Widget _buildSection(String title, IconData icon, String content, Color color) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text(content, style: const TextStyle(fontSize: 14, color: Colors.black87, height: 1.7)),
      ]),
    );
  }

  // ✅ NFR-SAFE-001: null이어도 기본 문구로 항상 표시
  Widget _buildDisclaimerCard() {
    final disclaimerText = _guide?['disclaimer'] as String? ??
        '본 안내문은 의료 진단을 대체하지 않습니다. 건강 관련 결정은 반드시 담당 의료진과 상담하시기 바랍니다.';
    return Container(
      width: double.infinity, padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 8),
        Text(disclaimerText, style: const TextStyle(color: Colors.grey, fontSize: 12, height: 1.6)),
      ]),
    );
  }
}