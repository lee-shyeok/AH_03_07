...
["카드1 핵심 문구", "카드2 핵심 문구", "카드3 핵심 문구"]"""
    ...
    async def create_card_news(self, user_id: UUID, guide_id: UUID) -> ContentConversionResponse:
        ...
        try:
            ...
            # 4. LLM으로 카드 텍스트 추출
            card_texts = await self.extract_card_texts(guide.guide_content)
    ...
    async def create_tts(self, user_id: UUID, guide_id: UUID) -> ContentConversionResponse:
        ...
        try:
            ...
            # 4. 텍스트 정제
            clean_text = self.clean_text_for_tts(guide.guide_content)
