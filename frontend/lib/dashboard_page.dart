...
class _DashboardPageState extends State<DashboardPage> {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            icon: const Icon(Icons.document_scanner_outlined, color: Color(0xFFFF8C00)),
            tooltip: 'OCR 내역',
            onPressed: _openOcrHistory,
          ),
          IconButton(
            icon: const Icon(Icons.chat_bubble_outline, color: Color(0xFFFF8C00)),
            tooltip: '챗봇',
            onPressed: _openChat,
          ),
        ],
  ...
  Widget _buildSectionTitle(String title, IconData icon, {VoidCallback? onMore}) {
    return Row(
      children: [
        ...
        if (onMore != null)
          GestureDetector(
            ...
            child: const Text('전체보기',
                style: TextStyle(fontSize: 12, color: Color(0xFFFF8C00), fontWeight: FontWeight.w600)),
          ),
      ],
    );
  }
      ...
      child: Column(
        ...
        children: [
          ...
          if (total == 0)
            ...
            Row(
              children: [
                Expanded(child: _buildMedicationStat('전체', total.toString(), const Color(0xFFFF8C00))),
                const SizedBox(width: 12),
                Expanded(child: _buildMedicationStat('남은 복약', remaining.toString(),
  ...
  Widget _buildRecentRecordsCard() {
    final records = (_data?['recent_records'] as List?)?.cast<Map<String, dynamic>>() ?? [];
    ...
    return _buildCard(
      child: Column(
        ...
        children: [
          ...
          if (records.isEmpty)
            const Text('최근 진료기록이 없습니다.', style: TextStyle(color: Colors.grey, fontSize: 14))
          else
  ...
  Widget _buildRecordItem(Map<String, dynamic> record) {
    ...
    try {
      formattedDate = DateFormat('yyyy.MM.dd').format(DateTime.parse(visitDate));
    } catch (_) {}
      ...
      child: Row(
        children: [
          Container(
            ...
            child: const Icon(Icons.local_hospital_outlined, color: Color(0xFFFF8C00), size: 20),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(hospitalName, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
          ...
          Text('약 $medCount종',
              style: const TextStyle(color: Color(0xFFFF8C00), fontSize: 12, fontWeight: FontWeight.bold)),
        ],
      ),
    );
  }
  ...
  Widget _buildRecentGuidesCard() {
    final guides = (_data?['recent_guides'] as List?)?.cast<Map<String, dynamic>>() ?? [];
  ...
  Widget _buildGuideItem(Map<String, dynamic> guide) {
    ...
    if (status == 'needs_update') { statusColor = Colors.red; statusText = '업데이트 필요'; }
  ...
  Widget _buildPendingOcrCard() {
    final jobs = (_data?['pending_ocr_jobs'] as List?)?.cast<Map<String, dynamic>>() ?? [];

    return _buildCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _buildSectionTitle('OCR 처리 내역', Icons.document_scanner_outlined, onMore: _openOcrHistory),
          const SizedBox(height: 12),
          if (jobs.isEmpty)
            const Text('처리 중인 문서가 없습니다.', style: TextStyle(color: Colors.grey, fontSize: 14))
          else
  ...
  Widget _buildOcrJobItem(Map<String, dynamic> job) {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          const Icon(Icons.insert_drive_file_outlined, color: Colors.grey, size: 20),
          const SizedBox(width: 8),
          Expanded(
            child: Text(filename, style: const TextStyle(fontSize: 14), overflow: TextOverflow.ellipsis),
          ),
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
            decoration: BoxDecoration(
