...
export default function MyPage() {
  ...
  const healthMenus = isAuto
    ...
    : [
        ...
        { href: "/documents", label: "문서 보관함", icon: Folder },
        { href: "/diet", label: "식단 가이드", icon: BookOpen },
        { href: "/pharmacy", label: "약국 찾기", icon: MapPin },
  ...
  return (
    <main className="mx-auto w-full max-w-md px-5 pb-6 pt-8">
      ...
      <span className="mt-3 inline-block rounded-full px-3 py-1 text-xs font-bold text-white" style={{ background: isAuto ? PURPLE : "hsl(var(--primary))" }}>
        {isAuto ? "자가면역" : "일반"}
      </span>
...
function StatBox({ label, value, auto }: { label: string; value: string; auto: boolean }) {
  return (
    <div className="rounded-xl px-4 py-3 text-center" style={{ background: auto ? PURPLE + "12" : "hsl(var(--secondary))" }}>
...
function Section({ title, menus }: { title: string; menus: { href: string; label: string; icon: typeof FileText }[] }) {
  return (
    <>
...
function Row({ icon: Icon, label, onClick, danger, border }: { icon: typeof FileText; label: string; onClick: () => void; danger?: boolean; border?: boolean }) {
  return (
    <button onClick={onClick} className={"flex w-full items-center gap-3 bg-card px-4 py-3.5 hover:bg-accent " + (border ? "border-t border-border " : "")}>
      <Icon className={"h-5 w-5 " + (danger ? "text-destructive" : "text-foreground")} strokeWidth={2} />
