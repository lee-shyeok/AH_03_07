...
@router.get("/users/me/consents", response_model=list[ConsentResponse], summary="API-사용자-004: 동의 이력 조회")
def get_consents(user_id: int = Depends(get_current_user_id), db: Session = Depends(get_db)):
    return (
        ...
        .order_by(UserConsent.consent_type, UserConsent.updated_at.desc())
        .all()
    )


@router.post("/users/me/consents", response_model=ConsentResponse, summary="API-사용자-005: 동의 추가·갱신")
def upsert_consent(
    data: ConsentUpsertRequest, user_id: int = Depends(get_current_user_id), db: Session = Depends(get_db)
):
...
@router.get("/users/me/mode", response_model=UserModeResponse, summary="API-모드-001: 현재 모드 조회")
def get_mode(user_id: int = Depends(get_current_user_id), db: Session = Depends(get_db)):
    return _get_or_create_mode(user_id, db)


@router.put("/users/me/mode", response_model=UserModeResponse, summary="API-모드-002: 모드 전환")
def update_mode(data: UserModeUpdate, user_id: int = Depends(get_current_user_id), db: Session = Depends(get_db)):
    if data.mode == UserModeEnum.autoimmune:
        ...
        if not consent:
            raise HTTPException(
                status_code=403, detail="자가면역 모드 사용을 위해 민감 의료정보 처리 동의가 필요합니다."
            )
...
@router.post("/diseases", response_model=UserDiseaseResponse, status_code=201, summary="API-질환-001: 질환 등록")
def create_disease(data: UserDiseaseCreate, user_id: int = Depends(get_current_user_id), db: Session = Depends(get_db)):
    count = (
        db.query(UserDisease)
        .filter(
            UserDisease.user_id == user_id,
    ...
    if count >= MAX_DISEASES_PER_USER:
        raise HTTPException(status_code=400, detail=f"질환은 최대 {MAX_DISEASES_PER_USER}개까지 등록할 수 있습니다.")
...
@router.get("/diseases", response_model=list[UserDiseaseResponse], summary="API-질환-002: 내 질환 목록 조회")
...
@router.put("/diseases/{disease_id}", response_model=UserDiseaseResponse, summary="API-질환-003: 질환 정보 수정")
def update_disease(
    disease_id: int, data: UserDiseaseUpdate, user_id: int = Depends(get_current_user_id), db: Session = Depends(get_db)
):
...
@router.delete("/diseases/{disease_id}", status_code=204, summary="API-질환-004: 질환 삭제")
def delete_disease(disease_id: int, user_id: int = Depends(get_current_user_id), db: Session = Depends(get_db)):
    disease = _get_disease_or_404(disease_id, user_id, db)
